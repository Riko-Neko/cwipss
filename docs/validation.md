...
- `validation_reviewed.csv`: generated by `scripts/run_stats.py` with
  Benjamini-Hochberg q-values and evidence ranks.
...
- The shuffle/null test is candidate-local; `scripts/run_stats.py` adds
  run-level and input-level Benjamini-Hochberg correction.
